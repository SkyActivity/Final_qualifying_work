...
from .models import ApplicantRecord, ApplicantMapRecord, StudentRecord
import openpyxl
...
def dashboard(request):
    ...
    total_specialties = StudentRecord.objects.values('specialty').distinct().count()
    total_regions = StudentRecord.objects.values('region').distinct().count()
    avg_score = StudentRecord.objects.aggregate(Avg('avg_score'))['avg_score__avg']
...
def build_specialty_chart(chart_type='bar', specialty_type=None):
    ...
    if data:
        # Одна запись на (специальность, форма обучения) — берём данные за последний год, чтобы не суммировать места по годам
        by_key = {}
        ...
        if chart_type == 'bar':
            fig = px.bar(
                ...
                hover_data=['Уровень образования'],
            ...
            fig = go.Figure()
            fig.update_layout(title='Тип графика не поддерживается', xaxis_title='Специальность', yaxis_title='Значение')
    ...
    if request.method == 'POST' and request.FILES['file']:
        ...
        try:
            ...
            if not headers_ok:
                ...
                return JsonResponse({'status': 'error', 'message': f'Неверные заголовки. Ожидаются: {expected_list}'}, status=400)
            ...
            # Для проверки: у одной специальности (форма, уровень, год) везде должны быть одинаковые проходной балл и места
            ...
            for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                ...
                if len(row) < len(EXPECTED_DATA_HEADERS_RU):
                    return JsonResponse({'status': 'error', 'message': f'Недостаточно колонок в строке {row_num}: {row}'}, status=400)
                ...
                year = to_int(row[16])
                if year is None:
                    return JsonResponse({'status': 'error', 'message': f'Строка {row_num}: не указан год поступления. Укажите год в колонке «Год поступления».'}, status=400)

                if is_blank(fio) or is_blank(gender) or is_blank(city) or is_blank(region):
                    return JsonResponse({'status': 'error', 'message': f'Строка {row_num}: некорректные данные (ФИО, пол, город, регион).'}, status=400)
                normalized_gender = normalize_gender(gender)
                if normalized_gender == 'U':
                    return JsonResponse({'status': 'error', 'message': f'Строка {row_num}: некорректный пол.'}, status=400)
                if birth_date is None:
                    return JsonResponse({'status': 'error', 'message': f'Строка {row_num}: некорректная дата рождения.'}, status=400)
                if is_blank(profile_subject):
                    return JsonResponse({'status': 'error', 'message': f'Строка {row_num}: не указан профильный предмет.'}, status=400)
                if None in (math_score, russian_score, profile_score, passing_score, budget_seats, paid_seats):
                    return JsonResponse({'status': 'error', 'message': f'Строка {row_num}: некорректные числовые значения (баллы или места).'}, status=400)
                if is_blank(specialty_code) or is_blank(specialty) or is_blank(learning_form) or is_blank(education_level):
                    return JsonResponse({'status': 'error', 'message': f'Некорректные данные по специальности в строке {row_num}: {row}'}, status=400)

                # Проверка: для одной специальности (форма, уровень, год) проходной балл и места должны совпадать во всех строках
                consistency_key = (
                ...
                aggregate['count'] += 1
                # Места не суммируем и не берём max: у одной специальности (форма, год) одно значение — берём из первой строки
                # (в Excel колонки мест повторяются у каждого ученика, но значение должно быть одно на специальность)
...
def build_student_charts(specialty=None, gender=None):
    ...
    male_counts = [int(age_gender[(age_gender['Возраст'] == age) & (age_gender['Пол'] == 'M')]['Количество'].sum()) for age in ages]
    female_counts = [int(age_gender[(age_gender['Возраст'] == age) & (age_gender['Пол'] == 'F')]['Количество'].sum()) for age in ages]
    ...
    fig_scatter = px.scatter(
        ...
        color_discrete_map={'Поступил': '#2ca02c', 'Не поступил': '#d62728'}
    )
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                ...
                if len(row) < 4:
                    ...
                    return JsonResponse({'status': 'error', 'message': f'Row {row} has insufficient columns.'}, status=400)
                ...
                if is_blank(city) or is_blank(region) or is_blank(specialty_code) or count is None:
                    return JsonResponse({'status': 'error', 'message': f'Некорректные данные в строке: {row}'}, status=400)
    ...
    if request.method == 'POST' and request.FILES['file']:
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                ...
                if birth_date is None:
                    return JsonResponse({'status': 'error', 'message': f'Некорректная дата рождения в строке: {row}'}, status=400)
                today = date.today()
                age = today.year - birth_date.year - ((today.month, today.day) < (birth_date.month, birth_date.day))
                if None in (math_score, russian_score, profile_score, passing_score):
                    return JsonResponse({'status': 'error', 'message': f'Некорректные баллы в строке: {row}'}, status=400)
                if avg_score is None:
                    avg_score = round((math_score + russian_score + profile_score) / 3, 2)
                if is_blank(specialty_code) or is_blank(specialty) or is_blank(learning_form) or is_blank(education_level):
                    return JsonResponse({'status': 'error', 'message': f'Некорректные данные по специальности в строке: {row}'}, status=400)
...
def get_coordinates_and_save(city, region):
    ...
    # Добавляем User-Agent, так как Nominatim требует его
    headers = {
...
def analytics_years(request):
    ...
    # Показываем только годы, в которых есть студенты (из Excel/данных), чтобы не показывать 2025 и т.п.
    ...
    # Бюджет местный больше не используется
    fig_trends.add_trace(go.Scatter(
...
def analytics_directions(request):
    ...
    # Бюджетные и платные места: у одной специальности несколько записей (Бакалавриат/Магистратура, формы, годы).
    # Показываем места по тому варианту (уровень + форма), где учится больше всего студентов.
    applicant_map = {}
    ...
    # График распределения по специальностям
    ...
    fig_bar.update_layout(
        ...
        yaxis_title='Специальность',
        template='plotly_white',
        height=max(600, len(specialties) * 30),
        margin=dict(l=200)
    )
    
    # Круговая диаграмма по формам обучения
    ...
    context = {
        ...
        'specialties_data': specialties_data[:10],  # Топ-10 для таблицы
    }
...
def analytics_regions(request):
    ...
    # Берем только регионы, которые есть в Data.xlsx (StudentRecord)
    ...
    # График по регионам: тёмный столбик = всего абитуриентов, синий = поступило по баллам
    ...
    # График средних баллов по регионам
